...
calBodyData,calBodySize = readInputFile(filename + '-calbody.txt')
calReadData,calReadSize = readInputFile(filename + '-calreadings.txt')
N_D = calBodySize(1)
N_A = calBodySize(2)
N_C = calBodySize(3)
...
c = calBodyData[np.arange(N_D + N_A + 1,end()+1),:]
...
for i in np.arange(1,N_framescal+1).reshape(-1):
    for j in np.arange(1,N_C+1).reshape(-1):
        C_exp[j,:,i] = np.transpose((inv(R_D[:,:,i]) * (R_A[:,:,i] * np.transpose(c(j,:)) + p_A[:,i] - p_D[:,i])))
